...
- Emotionally warm scene showing connection across time.`;
    ...
    // I'm making the error detection more robust. The API can send different
    // kinds of errors when it's under heavy load, so I'll check for more
    // keywords to correctly identify the situation for the user.
    const errorString = String(error).toLowerCase();
    if (
      errorString.includes('quota') ||
      errorString.includes('usage limit') ||
      errorString.includes('resource has been exhausted') || // Common for Google Cloud APIs
      errorString.includes('429') // HTTP code for "Too Many Requests"
    ) {
      throw new Error(
        "We're experiencing high demand right now. Please try again in a little while! 💜"
      );
    }
    
    // For any other error, I'll try to pass on a clear message.
    const details =
      (error as any).cause?.error?.message ||
      (error as Error).message ||
      'An unexpected error occurred.';
    throw new Error(`Failed to generate photo: ${details}`);
  }
};
